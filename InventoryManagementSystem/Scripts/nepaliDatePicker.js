  ...
  $.fn.nepaliDatePicker = function (options) {
    var datePickerPlugin = {
      ...
      getCalendar: function ($nepaliDatePicker) {
        ...
        calendarTable.append(datePickerPlugin.getCalendarFooter());
      ...
      getCalendarController: function ($nepaliDatePicker) {
        ...
        datePickerPlugin.getEngMonthsYear($nepaliDatePicker);
        calendarController.append(datePickerPlugin.getEngMonthsYear($nepaliDatePicker));
      ...
      getEngMonthsYear: function ($nepaliDatePicker) {
          var datePickerData = $nepaliDatePicker.data();
          var adStartmonth = calendarFunctions.getAdDateByBsDate(datePickerData.bsYear, datePickerData.bsMonth, 1).getMonth();
          var adEndMonth = calendarFunctions.getAdDateByBsDate(datePickerData.bsYear, datePickerData.bsMonth, 28).getMonth();
          var adStartYear = calendarFunctions.getAdDateByBsDate(datePickerData.bsYear, datePickerData.bsMonth, 1).getFullYear();
          var adEndYear = calendarFunctions.getAdDateByBsDate(datePickerData.bsYear, datePickerData.bsMonth, 28).getFullYear();

          if (adStartYear ==adEndYear) {

              return "<i>"+calendarData.adMonths[adStartmonth]+"/"+calendarData.adMonths[adEndMonth]+"-"+adStartYear +"</i>";
          }
          else {

              return "<i>"+calendarData.adMonths[adStartmonth]+"/"+calendarData.adMonths[adEndMonth]+"-"+adStartYear +"/"+adEndYear.toString().slice(-2)+"</i>";
          }
      ...
      getCalendarFooter: function () {
          var calendarFooter = $("<tr>");
         
          calendarFooter.append('<td  colspan="7" style="text-align:center !important border:none; padding:5px"> <a href="javascript:void(0);" class="today-btn" title=""> Today </a> </td>');
            ...
            if (isCurrentMonthDate) {
              ...
              var $td = $(
                '<td class="current-month-date" data-date="' +
                  calendarDate +'" data-weekDay="' +(k - 1) +'">' +
                  calendarFunctions.getNepaliNumber(calendarDate) +
                   '<i style="padding:2px; font-size:8px"> ' + calendarFunctions.getAdDateByBsDate(bsYear, bsMonth, calendarDate).getDate() + ' </i>' +
                  '</td>'
