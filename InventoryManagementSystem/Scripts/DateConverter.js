...
function countBSDays(date) {
  ...
  for (var j = start.year; j <= end.year; j++) {
    dayCount += calendar_data[j][12];
  }
...
function getBS(date) {
  var bs = ad2bs(date);
  if (bs.en.month < 10) {
    bs.en.month = "0" + bs.en.month;
  }
  if (bs.en.day < 10) {
    bs.en.day = "0" + bs.en.day;
  }
  return bs.en.year + "/" + bs.en.month + "/" + bs.en.day;
}
...
function getAD(date) {
  ...
  return ad.year + "/" + ad.month + "/" + ad.day;
}
